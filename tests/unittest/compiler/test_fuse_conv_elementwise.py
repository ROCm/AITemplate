...
class FuseConvBiasCase(unittest.TestCase):
    ...
    def test_conv2d_bias_fastgelu(self):
        B = [1]
        batch_dim = shape_utils.gen_int_var_min_max(B, name="batch_dim")
        CO, HH, WW, CI = 256, 28, 28, 128
        filter_HW = 3
        ...
        conv2d_bias_fastgelu = ops.elementwise(FuncEnum.FASTGELU)(conv2d_bias)
        conv2d_bias_fastgelu._attrs["is_output"] = True
        conv2d_bias_fastgelu._attrs["name"] = "output_0"

        target = detect_target()
        module = compile_model(
            conv2d_bias_fastgelu, target, "./tmp", "test_conv2d_bias_fastgelu"
        )
        ...
        self.assertEqual(len(check_tensor.src_ops()), 1)
        src_op = list(check_tensor.src_ops())[0]
        self.assertEqual(src_op._attrs["op"], "conv2d_bias_fastgelu")
        ...
        for b in B:
            ...
            Y_pt = torch.nn.functional.conv2d(X_pt, W_pt, padding=1)
            Y_pt = Y_pt + B_pt
            Y_pt = torch.nn.functional.gelu(Y_pt)
