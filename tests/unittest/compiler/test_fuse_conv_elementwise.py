...
class FuseConvBiasCase(unittest.TestCase):
    ...
    def test_conv2d_bias_silu(self):
        ...
        conv2d_bias_silu = ops.elementwise(FuncEnum.SILU)(conv2d_bias)
        conv2d_bias_silu._attrs["is_output"] = True
        conv2d_bias_silu._attrs["name"] = "output_0"

        target = detect_target()
        module = compile_model(
            conv2d_bias_silu, target, "./tmp", "test_conv2d_bias_silu"
        )
        ...
        self.assertEqual(len(check_tensor.src_ops()), 1)
        src_op = list(check_tensor.src_ops())[0]
        self.assertEqual(src_op._attrs["op"], "conv2d_bias_silu")
